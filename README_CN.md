...
**[GPA - ASR、TTS 和声音转换三合一模型](https://github.com/AutoArk/GPA)**

一个统一的音频模型，将 ASR（自动语音识别）、TTS（文本转语音）和声音转换功能集成在仅 **0.3B 参数**的模型中。该模型专为以下场景设计：
- **端侧部署**：轻量级模型，适合移动设备和边缘计算
- **商业服务**：针对大规模生产部署进行了优化
- **一站式解决方案**：单个模型即可实现语音识别、合成和声音转换

如果您需要更紧凑、多功能的端侧或商业音频解决方案，建议探索 GPA 项目。
